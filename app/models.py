...
def get_company_by_ein(ein):
    conn = get_connection()
    row = conn.execute("SELECT * FROM companies WHERE ein=?", (ein,)).fetchone()
    conn.close()
    return row
...
def search_provider_directory(name):
    conn = get_connection()
    rows = conn.execute("SELECT * FROM provider_directory WHERE legal_name LIKE ? OR display_name LIKE ?",
...
def get_dataset_files(year):
    conn = get_connection()
    rows = conn.execute("SELECT * FROM dataset_files WHERE dataset_year=?", (year,)).fetchall()
    conn.close()
    return rows